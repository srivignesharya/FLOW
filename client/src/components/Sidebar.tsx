...
export const Sidebar: React.FC = () => {
  ...
  return (
    <motion.aside
      ...
      className="relative bg-slate-900/90 border-r border-slate-800 flex flex-col h-screen sticky top-0 overflow-hidden z-20 backdrop-blur-xl shadow-2xl"
    >
      {/* Logo */}
      <div className={clsx(
        'flex items-center border-b border-slate-800 h-16 px-4 gap-3 shrink-0',
              ...
              <>
                {isActive && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-brand-500/10 border border-brand-500/30 rounded-xl shadow-sm shadow-brand-500/20"
                    transition={{ type: 'spring', stiffness: 400, damping: 30 }}
                  />
                )}
                <link.icon className={clsx(
                  'relative z-10 h-5 w-5 shrink-0 transition-transform duration-200 group-hover:scale-110',
                  isActive ? 'text-brand-400' : 'group-hover:text-slate-100'
                )} />
                <AnimatePresence>
                  {!collapsed && (
                    <motion.span
      ...
      <AnimatePresence>
        {!collapsed && (
          ...
          >
            <div className="px-3 py-2.5 rounded-xl bg-slate-950/60 border border-brand-500/20">
              <p className="text-xs font-extrabold bg-gradient-to-r from-brand-400 to-amber-400 bg-clip-text text-transparent">FLOW AI SaaS</p>
              <p className="text-[11px] text-slate-400 mt-0.5 font-medium">Powered by Gemini & IMvision</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
