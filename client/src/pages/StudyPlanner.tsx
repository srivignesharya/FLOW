    ...
    <div className="space-y-8 animate-in max-w-5xl mx-auto">
      ...
      {generating ? (
        ...
        <div className="space-y-6">
          ...
          <div className="card p-6 bg-gradient-to-r from-brand-950/40 via-slate-900 to-indigo-950/40 border-brand-900/50">
          ...
          <div className="space-y-6">
            {plan.dailyPlans?.map((dayPlan: any, dayIdx: number) => (
              <div key={dayIdx} className="card p-6 space-y-4 relative overflow-hidden">
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                  <div className="flex items-center gap-3">
                    <div className="p-2.5 bg-brand-50 dark:bg-brand-950/60 text-brand-600 dark:text-brand-400 rounded-xl font-bold">
                    ...
                    <div>
                      ...
                      <span className="block text-xs text-slate-400">Day {dayIdx + 1} of 7</span>
                    </div>
                  </div>
                  ...
                  {dayPlan.blocks?.map((block: any, bIdx: number) => (
                    ...
                    >
                          ...
                          <button
                            ...
                            className={`h-5 w-5 rounded-md border flex items-center justify-center transition-colors shrink-0 ${
                              block.completed
                                ...
                                : 'border-slate-400 dark:border-slate-600 hover:border-brand-500'
                            }`}
                            title="Mark study session completed"
                          >
                            {block.completed && <CheckCircle2 className="h-3.5 w-3.5" />}
                          </button>
                          <span className={`font-bold text-sm text-slate-900 dark:text-slate-100 ${block.completed ? 'line-through text-slate-400' : ''}`}>
                            {block.taskTitle}
                          </span>
                        </div>
                      ...
                      <div className="flex items-center justify-between text-xs text-slate-400 pt-1">
                        <span className="font-semibold text-indigo-400">{block.subject}</span>
